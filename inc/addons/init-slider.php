...
if ( ! function_exists( 'the_slick_gallery' ) ) {
	 ...
	 * @param string $slick_init   slick init params.
	 ...
	 * @param string $columns_count number of columns. Default: 1.
	 ...
	 * @return array
	 */
	function the_slick_gallery( $folder = 'data/', $slick_id = null, $slick_init = null, $titles = array(), $columns_count = 1, $inner = true, $shuffle = true, $allowed_types = array( 'jpg', 'png', 'gif', 'jpeg', 'webp' ), $caption_text = null, $caption_link = null, $echo = true ) {
		...
		$html .= '<div class="slick-container">';
			...
			foreach ( $images as $key => $image ) {
				...
				$html .= '<a class="slick-item" href="' . esc_url( $image ) . '" aria-hidden="true" tabindex="-1">';
					$html .= '<div class="slick-image" style="background: url( ' . esc_url( $image ) . ' ) center/contain no-repeat" role="img"></div>';
				$html .= '</a>';
...
/*		wp_enqueue_style( 'swiper-styles' );
		...
		if ( is_null( $slick_init ) ) {
			$slick_init = "const swiper = new Swiper('.swiper', {
				loop: true,
				pagination: {
		...
		wp_add_inline_script( 'swiper-scripts', minify_js( $slick_init ) );
*/
