...
// Добавляем настройки по умолчанию на after_setup_theme хук
if ( !function_exists( 'wpgen_setup' ) ) {
	function wpgen_setup() {
		...
		// Добавляем дефолтный RSS в <head>
		add_theme_support( 'automatic-feed-links' );

		// Let WordPress manage the document title
		// Подключаем тег title в <head>
		add_theme_support( 'title-tag' );

		// Enable support for Post Thumbnails on posts and pages
		// Включаем поддержку thumbnail для постов и страниц
		add_theme_support( 'post-thumbnails' );

		// This theme uses wp_nav_menu() in one location
		// Регистрируем primary меню
		register_nav_menus( array(
			'primary' => esc_html__( 'Primary', 'wpgen' ),
		) );

		// Switch default core markup for search form, comment form, and comments
		// Подключаем дефолтную html разметку для формы поиска, формы комментариев и галерей
		...
		// Добавляем поддержку post-formats
		add_theme_support( 'post-formats', array( 'aside', 'image', 'video', 'quote', 'link') );

		// Set up the WordPress core custom background feature
		// Устанавливаем настройки по умолчанию для custom-background
		...
		// Включаем поддержку «Selective Refresh» (выборочное обновление) для виджетов в кастомайзере
		add_theme_support( 'customize-selective-refresh-widgets' );

		// Set the content width in pixels, based on the theme's design and stylesheet
		// Устанавливаем ширину содержимого в пикселях, основываясь на дизайне темы и таблице стилей
		$GLOBALS['content_width'] = apply_filters( 'wpgen_content_width', 960 );

		// Add support for core custom logo
		// Добавляем поддержку кастом logo
...
// Подключаем основные стили и скрипты
function wpgen_scripts() {
	...
	// Сетка Бутстрап
	wp_enqueue_style( 'bootstrap-grid', get_theme_file_uri( 'assets/css/bootstrap-grid.min.css' ) );
...
function wpgen_widgets_init() {
	...
	if ( true == wpgen_options( 'general_top_bar_display' ) ) {
		register_sidebar( array(
	...
	if ( true == wpgen_options( 'general_bottom_bar_display' ) ) {
		register_sidebar( array(
	...
	if ( 'footer-three-columns' == wpgen_options( 'general_footer_type' ) || 'footer-four-columns' == wpgen_options( 'general_footer_type' ) ) {
		register_sidebar( array(
	...
	if ( 'footer-four-columns' == wpgen_options( 'general_footer_type' ) ) {
		register_sidebar( array(
